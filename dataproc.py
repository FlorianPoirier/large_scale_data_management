...
INIT = Pig.compile("""
A = LOAD 'gs://chicken-lava-bucket/small_page_links.nt' using PigStorage(' ') as (url:chararray, p:chararray, link:chararray);
B = GROUP A by url;                                                                                  
...
params = { 'd': '0.85', 'docs_in': 'gs://chicken-lava-bucket/out/pagerank_data_simple' } #modified param from 0.5 to 0.85
...
for i in range(3):
   out = "gs://chicken-lava-bucket/out/pagerank_data_" + str(i + 1)
   params["docs_out"] = out
   Pig.fs("rmr " + out)
